...
public class Board {
	...
	public boolean hasWon(Player thePlayer, int playerRow, int playerCol) {
		...
		//  check if the player has 3-in-the-reverse-diagonal
	...
	public void paint(Graphics g) {
		...
		for (int row = 1; row < GameMain.ROWS; ++row) {          
			g.fillRoundRect(0, GameMain.CELL_SIZE * row - GRID_WIDTH_HALF,                
				GameMain.CANVAS_WIDTH - 1, GRID_WIDTH, GRID_WIDTH, GRID_WIDTH);       
		}
		
		for (int col = 1; col < GameMain.COLS; ++col) {          
			g.fillRoundRect(GameMain.CELL_SIZE * col - GRID_WIDTH_HALF, 0,                
				GRID_WIDTH, GameMain.CANVAS_HEIGHT - 1, GRID_WIDTH, GRID_WIDTH);
		}
